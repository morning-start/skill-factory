...
layer: 2
phase: processing
description: 技能丰富器，补充示例、references、能力、Mermaid 图表
tags: [skill-factory, enricher, enhancement]
dependency:
  parent: skill-factory-phase-processing
---
...
本加工器在以下策略中使用：
- ✅ **精简优先 (Simplify-First)**: 第2步（选择性补充）
- ✅ **丰富优先 (Enrich-First)**: 第1步（核心操作）
- ✅ **均衡模式 (Balanced)**: 第2步（按需补充）
- ❌ **快速路径**: 不使用（跳过加工）

**与其他加工器的协作关系**:
| 关系 | 加工器 | 说明 |
|------|--------|------|
| 前置依赖 | 无 / Simplifier | 精简优先时先执行 simplifier |
| 后续触发 | Beautifier | 丰富后建议美化 |
| 最终校验 | Standardizer | 所有策略的最后一步 |
