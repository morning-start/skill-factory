...
layer: 2
phase: processing
description: 技能规范化器，前言区校准、命名规范、必备章节、链接检查
tags: [skill-factory, standardizer, compliance]
dependency:
  parent: skill-factory-phase-processing
---
...
本加工器在以下策略中使用：
- ✅ **精简优先 (Simplify-First)**: 第3步（最终校验）
- ✅ **丰富优先 (Enrich-First)**: 第3步（规范化）
- ✅ **均衡模式 (Balanced)**: 第4步（最终校验）⭐ 必须步骤
- ❌ **快速路径**: 不使用（跳过加工）
...
|------|--------|------|
| 前置依赖 | 所有加工器 | 作为最后一步 |
| 质量把关 | Packager | 生产阶段的结构验证 |
| 输出保证 | Publisher | 确保符合发布标准 |

**特殊说明**: Standardizer 是所有策略的**必经步骤**，负责最终质量把关。
