...
基于对 [Superpowers](https://github.com/obra/superpowers) 项目的深度分析，融合其核心方法论，实现技能创建质量的质的飞跃。
...
- ✨ **RED 阶段（观察失败）**:
  - 压力场景设计（时间压力/权威压力/疲劳/过度自信）
  - 基线测试执行（用子代理无技能时测试）
  - 违规行为和合理化借口逐字记录
  - 模式识别与总结
- ✨ **GREEN 阶段（编写技能）**:
  - 最小可行技能原则（只解决观察到的问题）
  - 合规性验证（Agent 在有技能时遵守规则）
- ✨ **REFACTOR 阶段（修补漏洞）**:
  - 发现新合理化方式并逐一禁止
  - 重测试至全部通过
  - 合理化对照表输出

#### 🆕 核心新增：CSO 发现优化（Claude Search Optimization）

**核心陷阱**: description 写工作流 → Agent 跳过正文

- ✨ **Description 编写规则**（8 条）:
  - 以 "Use when..." 开头
  - 只写触发条件，不总结工作流
  - 具体症状和关键词覆盖
  - 第三人称、长度控制 <500 字符
- ✨ **Token 效率原则**:
  ...
  - 频繁加载技能: <200 词
  - 其他技能: <500 词
- ✨ **命名最佳实践**: 动词优先、动名词 (-ing)、按功能命名

#### 🆕 核心新增：反合理化设计模式（R10）

> 心理学基础：Cialdini 2021; Meincke et al., 2025

- ✨ **5 大反合理化技术**:
  1. 关闭每个漏洞显式化（"No exceptions:" 列表）
  2. 解决"精神 vs 字面"论点（违反字面 = 违反精神）
  3. 构建合理化对照表（借口 vs 现实 vs 应对）
  4. 创建红旗警告列表（🚩 Red Flags - STOP and Start Over）
  5. 更新 CSO 以包含违规症状
- ✨ **说服原则应用**: 权威、承诺、稀缺、社会证明、统一

#### 升级现有文件

| 文件 | 主要变更 |
|------|---------|
| **SKILL.md (根)** | v0.5.1→v0.6.0; 新增 TDD + CSO 章节; 设计模式 11→15; 规范清单 6→8 |
| **skills/creator/SKILL.md** | v0.5.1→v0.6.0; 融入 TDD Phase 0-2; CSO description; 压力场景模板 |
| **references/writing-rules.md** | v0.5.1→v0.6.0; 规则 7→10; 新增 R8(TDD)+R9(CSO)+R10(反合理化) |
| **references/skill-standards.md** | v0.5.1→v0.6.0; CSO description 规则(8条); TDD 验证检查(6项); 技能类型×测试策略矩阵 |
| **metadata.json** | v0.5.1→v0.6.0; 新增 tdd_methodology/cso_optimization 字段; key_concepts 扩展 |

#### 技能类型 × 测试策略矩阵（新增）

| 类型 | RED 测试方法 | GREEN 验证 | 成功标准 |
|------|------------|-----------|---------|
| **规范强制型** | 学术提问+压力场景+多重组合 | 最大压力下仍遵守 | 无法绕过规则 |
| **技术方法型** | 应用场景+边界情况+信息缺失 | 正确应用到新场景 | 边界情况正确使用 |
| **思维模式型** | 识别场景+反例测试 | 判断适用性 | 正确判断何时/如何用 |
| **参考文档型** | 检索场景+应用场景+空白测试 | 找到并正确使用 | 检索准确率 ≥90% |
...
| 文件数 | 7 | 7 | 不变 |
| 总行数 | ~1400 | **~2200** | +57% |
| 设计模式 | 11 | **15** | +4 (TDD/CSO/压力测试/复杂度分级) |
| 写作规则 | 7 | **10** | +3 (TDD/CSO/反合理化) |
| 规范检查项 | 6 | **8** | +2 (CSO/TDD) |

### 🙏 致谢

特别感谢 [Superpowers](https://github.com/obra/superpowers) 项目（by Jesse Vincent & Prime Radiant）的开源贡献，其 TDD 驱动创建和 CSO 优化方法论为本版本提供了核心灵感。
